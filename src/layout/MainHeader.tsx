        ...
        >
            ...
            <div className=" flex items-center gap-4">
                <Button
                    ...
                    className={`flex lg:hidden! items-center justify-center p-3 !text-slate-600 hover:!text-${colorPreset}-600 hover:!bg-${colorPreset}-50 !rounded-lg transition-all`}
                    onClick={onMenuClick}
                />

                {/* Search Bar - Responsive */}
                <div className={`flex items-center bg-slate-50 px-5 py-3 rounded-full border border-slate-100 w-full max-w-[200px] md:max-w-xs lg:w-96 hover:bg-slate-100 transition-colors focus-within:ring-2 focus-within:ring-${colorPreset}-100 focus-within:border-${colorPreset}-200`}>
            ...
            <div className="flex items-center gap-3 md:gap-6">
                <div className="flex items-center gap-2">
                    <Badge count={5} size="small" offset={[-2, 2]} className="cursor-pointer">
                        <div className={`w-10 h-10 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 hover:text-${colorPreset}-600 hover:bg-${colorPreset}-50 transition-all shadow-sm`}>
                ...
                <div className="flex items-center gap-3 cursor-pointer group p-1 px-4  rounded-lg hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100">
                    <div className="text-right hidden lg:block">
                        <p className={`text-slate-800 text-sm font-bold leading-tight group-hover:text-${colorPreset}-600 transition-colors`}>Test User</p>
                        <p className="text-slate-400 text-xs leading-tight font-medium">Administrator</p>
                    </div>
                    <div className={`p-0.5 h-12 w-12 flex items-center justify-center rounded-full bg-linear-to-tr ${accentGradient.from} ${accentGradient.to}`}>
                        <Avatar
                            size={40}
