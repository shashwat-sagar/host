...
const empaneledTypeId = 57;
const type = "Corporate";

const fetchCorporateList = async (t: string) => {
    const { data } = await getHospitalEmpanelledList(t);
    return data;
};

const fetchCorporateDetail = async (hospitalRegNo: any, typeId: number) => {
    const { data } = await getHospitalEmpanelledDetail(hospitalRegNo, typeId);
    return data?.table;
};

const fetchCorporateDetailById = async (hospitalRegNo: any, id: any) => {
...
const Corporate = () => {
    ...
    const [modalData, setModalData] = useState({
        ...
        empaneledTypeName: type,
    });

    const { data: corporateList, isFetching: corporateListLoading } = useQuery({
        queryKey: ["empanelledCorporateList", type],
        queryFn: () => fetchCorporateList(type),
        refetchOnWindowFocus: false,
    });

    const { data: corporateDetail, isFetched: corporateDetailFetched } = useQuery({
        queryKey: ["empanelledCorporateDetail", hospitalRegNo, empaneledTypeId],
        enabled: !!hospitalRegNo,
        queryFn: () => fetchCorporateDetail(hospitalRegNo, empaneledTypeId),
        refetchOnWindowFocus: false,
    });

    useEffect(() => {
        if (corporateDetailFetched && corporateDetail) {
            const initialValues = corporateDetail.reduce((acc: any, item: any) => {
                acc[item?.empaneledId] = true;
                return acc;
            }, {});
            form.setFieldsValue(initialValues);
        }
    }, [corporateDetailFetched, corporateDetail, form]);
    ...
    const {
        ...
        mutationFn: (id: any) => fetchCorporateDetailById(hospitalRegNo, id),
    });
    ...
    return (
        <>
            <Heading title="Corporate Empanelment" />
            <div className="p-4 bg-white rounded-xl shadow-sm m-2">
                <Form form={form} layout="vertical">
                    <MasterTable
                        ...
                        bordered
                        data={corporateList}
                        isLoading={corporateListLoading}
                        rowKey="value"
