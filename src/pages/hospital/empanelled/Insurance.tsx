...
const empaneledTypeId = 59;
const type = "insurance";

const fetchInsuranceList = async (t: string) => {
    const { data } = await getHospitalEmpanelledList(t);
    return data;
};

const fetchInsuranceDetail = async (hospitalRegNo: any, typeId: number) => {
    const { data } = await getHospitalEmpanelledDetail(hospitalRegNo, typeId);
    return data?.table;
};

const fetchInsuranceDetailById = async (hospitalRegNo: any, id: any) => {
...
const Insurance = () => {
    ...
    const [modalData, setModalData] = useState({
        ...
        empaneledTypeName: "Insurance",
    });

    const { data: insuranceList, isFetching: insuranceListLoading } = useQuery({
        queryKey: ["empanelledInsuranceList", type],
        queryFn: () => fetchInsuranceList(type),
        refetchOnWindowFocus: false,
    });

    const { data: insuranceDetail, isFetched: insuranceDetailFetched } = useQuery({
        queryKey: ["empanelledInsuranceDetail", hospitalRegNo, empaneledTypeId],
        enabled: !!hospitalRegNo,
        queryFn: () => fetchInsuranceDetail(hospitalRegNo, empaneledTypeId),
        refetchOnWindowFocus: false,
    });

    useEffect(() => {
        if (insuranceDetailFetched && insuranceDetail) {
            const initialValues = insuranceDetail.reduce((acc: any, item: any) => {
                acc[item?.empaneledId] = true;
                return acc;
            }, {});
            form.setFieldsValue(initialValues);
        }
    }, [insuranceDetailFetched, insuranceDetail, form]);
    ...
    const {
        ...
        mutationFn: (id: any) => fetchInsuranceDetailById(hospitalRegNo, id),
    });
    ...
    return (
        <>
            <Heading title="Insurance Empanelment" />
