...
import { Heading, SubHeading, MasterTable } from "../../components";
...
const HospitalSpecialization = () => {
    ...
    const { data: getSpecializationData, isFetching: isSpecTypeLoading } = useQuery({
    ...
    const { data: SpecializationFacilityData, isFetching: isFacilityListLoading } = useQuery({
    ...
    const { data: hospitalSpecialityData, isFetching: isDetailLoading } = useQuery({
    ...
    useEffect(() => {
        ...
        if (firstEntry) {
            radioValues["specializations"] = firstEntry.specializationTypeId?.toString();
                ...
                >
                    <Radio.Group buttonStyle="solid">
                        <Radio.Button value={1} className="px-6">
                            <Tag color="green" className="border-none bg-transparent m-0 p-0 text-inherit">YES</Tag>
                        </Radio.Button>
                        <Radio.Button value={0} className="px-6">
                            <Tag color="red" className="border-none bg-transparent m-0 p-0 text-inherit">NO</Tag>
                        </Radio.Button>
                    </Radio.Group>
                </Form.Item>
            ),
    ...
    const { mutate: saveHospiatlSpec, isPending: saveHospitalSpecPending } =
        useMutation({
            ...
            onSuccess: (data) => {
                if (data?.status === true) {
                    message.success(data?.message || "Specializations updated successfully");
                } else {
    ...
    return (
        <>
            <Heading title="Hospital Specializations" />
                        ...
                        <Col xs={24} md={12}>
                            ...
                            >
                                <Select
                                    ...
                                    prefix={<FaHandHoldingMedical className="text-green-600 mr-2" />}
                                />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={12}>
