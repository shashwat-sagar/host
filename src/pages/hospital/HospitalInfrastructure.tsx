...
const HospitalInfrastructure = () => {
                ...
                >
                    <Radio.Group buttonStyle="solid">
                        {hospitalInfrastructureAvailabilityOptions?.map((option: any) => (
                            <Radio.Button key={option.value} value={parseInt(option.value)} className="px-4">
                                <Tag
                                    color={
                                        option.value === "1"
                                            ? "green"
    ...
    const onFinish = async (values: any) => {
        const payload = Object.entries(values).map(([key, value]) => {
            const infraItem = hospitalInfrastructureList?.find((item: any) => item.value === key);
            return {
    ...
    return (
        <>
            <Heading title={"Hospital Infrastructure"} />
            <div className="p-4 bg-white rounded-xl shadow-sm m-2">
                ...
                >
                    <MasterTable
                        ...
                        bordered
                        data={hospitalInfrastructureList}
                        isLoading={hospitalInfrastructureListLoading || hospitalInfrastructureByIdLoading}
                        rowKey="value"
